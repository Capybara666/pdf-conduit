...
public class WebOperations {
    ...
    public List<NamedBytes> extractSeparate(NamedBytes in, String pagesExpr)
            throws PdfOperationException, InvalidPageRangeException {
        return extractSeparate(in, pagesExpr, outputBudget.tally());
    }

    /** As {@link #extractSeparate(NamedBytes, String)} but sharing one request-wide byte budget. */
    private List<NamedBytes> extractSeparate(NamedBytes in, String pagesExpr, OutputBudget.Tally tally)
            throws PdfOperationException, InvalidPageRangeException {
        byte[] pdf = routeToPdf(in);
        try (LoadedPdf lp = LoadedPdf.open(pdf)) {
            guardPageCount(lp);
            // Per-page split is multi-output: every page becomes its own PDF held in the heap, so
            // the run aborts (422) the moment the request's accumulated result outgrows the budget.
            List<byte[]> pages = PdfSplitter.separateBytes(pdf, range(pagesExpr, lp), tally.guard());
            tally.commit(pages);
    ...
    public List<NamedBytes> extractSeparate(List<NamedBytes> inputs, String pagesExpr)
        ...
        for (NamedBytes in : inputs) out.addAll(extractSeparate(in, pagesExpr, tally));
        return out;
    }
    ...
    public NamedBytes redact(NamedBytes in, List<RedactRegion> regions, int dpi, boolean reOcr)
            throws PdfOperationException {
        byte[] pdf = routeToPdf(in);
        // Only the pages carrying a non-zero region are rasterised (zero-area regions are a no-op
        // in PdfRedactor) — mirror that grouping so both the render budget below and the re-OCR
        // pass below count exactly the pages that really get rendered.
