...
- **Config keys** `pdfconduit.web.*` (`config/WebProperties`, all env-overridable): `soffice-path`, `max-files-per-request`, `cors.allowed-origins`, `office.{enabled,max-concurrent,timeout-seconds}`, `ratelimit.{enabled,requests-per-minute,heavy-per-minute,burst}`, `quota.{enabled,daily-operations,free-max-file-size,free-max-files}`, `concurrency.{max-heavy-ops,max-in-flight-bytes}`, `processing.timeout-seconds`, `pdf.max-pages`, `render.{max-dpi,max-output-pixels}`, `trusted-proxies`. Env-var name = property path upper-cased, `.`/`-`→`_` (e.g. `PDFCONDUIT_WEB_QUOTA_FREE_MAX_FILE_SIZE`).
